...
int main(void) {
    printf("--- SENARYO A: pNumConfig - Gecerli Isaretci ---\n\n");
    ...
    if (aktarilan_sayi <= 0) {
        printf("Hata: pNumConfig gecerli olsa da bu EGLDisplay icin config sayisi 0 geldi.\n");
        printf("GORSEL SONUC: Config olmadigi icin context/surface kurulmadan program temiz kapaniyor.\n");
        eglTerminate(egl_dpy);
        destroy_native_display(native_ctx);
        return 0;
    }
    ...
    if (!secilen_config) {
        printf("Hata: Gorsel cikti icin uygun config bulunamadi.\n");
        eglTerminate(egl_dpy);
        destroy_native_display(native_ctx);
        return 1;
    }
    ...
    glClearColor(0.24f, 0.10f, 0.28f, 1.0f);
    glClear(GL_COLOR_BUFFER_BIT);
    reset_common_gl_objects();
    draw_triangle(0.0f, 0.0f, 1.0f, 1.0f);

    printf("BASARILI: pNumConfig gecerli isaretci oldugu icin EGL yazdigi config sayisini bildirdi: %d\n",
           aktarilan_sayi);
    printf("GORSEL SONUC: Uygun config ile mor zemin uzerine camgobegi ucgen cizildi.\n");
