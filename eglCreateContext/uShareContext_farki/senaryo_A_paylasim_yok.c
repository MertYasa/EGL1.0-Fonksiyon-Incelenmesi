...
int main() {
    ...
    if (main_ctx == EGL_NO_CONTEXT || isolated_ctx == EGL_NO_CONTEXT) {
        log_egl_error("eglCreateContext");
        eglDestroySurface(display, surface);
        if (main_ctx != EGL_NO_CONTEXT) eglDestroyContext(display, main_ctx);
        if (isolated_ctx != EGL_NO_CONTEXT) eglDestroyContext(display, isolated_ctx);
    ...
    if (!eglMakeCurrent(display, surface, surface, isolated_ctx)) {
        log_egl_error("eglMakeCurrent(isolated_ctx)");
        eglDestroyContext(display, isolated_ctx);
        eglDestroyContext(display, main_ctx);
        eglDestroySurface(display, surface);
        gbm_surface_destroy(win);
    ...
    draw_quad(-0.9f, -0.9f, 0.9f, -0.55f, 0.0f, 0.9f, 0.0f, 0.0f);
    draw_quad(-0.9f, 0.55f, 0.9f, 0.9f, 0.0f, 0.9f, 0.0f, 0.0f);

    printf("\nDEGER A: share_context = EGL_NO_CONTEXT\n");
    printf(" -> SONUC: 2. baglam 1. baglamin dokusunu %s.\n", is_valid ? "TANIYOR" : "TANIMIYOR");
    printf(" -> GORSEL SONUC: Texture paylasilamadigi icin ekranda kirmizi hata/desen alani gorulur.\n\n");
