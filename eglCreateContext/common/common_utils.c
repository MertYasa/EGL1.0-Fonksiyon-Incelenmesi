...
NativeDisplayContext* init_native_display_at(int connected_index) {
    ...
    ctx->crtc_id = resources->count_crtcs > 0 ? resources->crtcs[0] : 0;

    if (connector->encoder_id) {
        drmModeEncoder *encoder = drmModeGetEncoder(ctx->fd, connector->encoder_id);
        if (encoder) {
            if (encoder->crtc_id) {
...
void present_native_display(NativeDisplayContext* ctx, struct gbm_surface* surface) {
    present_native_display_for(ctx, surface, 0);
}
