...
int main() {
    ...
    NativeWindow nw;
    if (!create_drm_window(&nw, 800, 600, "EGLTerminate - Senaryo B (Hatali Display)")) {
        return -1;
    }

    printf("DRM/KMS Ekrani acildi. Simdi hatali parametre (EGL_NO_DISPLAY) kullanilarak eglTerminate cagrilmaya calisiliyor...\n");
    ...
    if (result == EGL_FALSE) {
        ...
        if (err == EGL_BAD_DISPLAY) {
            ...
            printf("Gorsel Kanit: Ekranda sadece bos, siyah bir DRM/KMS ekrani gorunmektedir (3 saniye sonra kapanacak).\n");
        }
    } else {
